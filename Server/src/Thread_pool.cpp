...
void *thread_pool::Manager(void *arg)
{
    ...
    while(p->thread_shutdown )
    {
        ...
        cur = p->queue_cur;
        pthread_mutex_unlock(&p->lock);
        printf("====================Current queue_size : %d====================\n",cur);
        printf("====================Current alive_thread : %d====================\n",alive);
        printf("====================Current busy_thread : %d====================\n",busy);
        ...
        if(busy *2 < alive - cur && alive > p->thread_min)
        {
            printf("lalalalalal\n");
            pthread_mutex_lock(&p->lock);
...
int thread_pool::GetAlive(pool_t* p)
{
    int alive = 0;
    pthread_mutex_lock(&p->lock);
    alive = p->thread_alive;
...
int thread_pool::GetBusy(pool_t* p)
{
    int busy = 0;
    pthread_mutex_lock(&p->lock);
    busy = p->thread_busy;
